"""Style cluster detection and document classification.

detect_styles       — detection pass: calls models, consolidates, returns StyleCluster list
...
def detect_styles(
    ...
    """Run detection pass: stratified sample → detection models → StyleCluster list.

    detection_models:
        None / []  = style-weighted subset (claude + openai by default)
    ...
    if not sample:
        log.warning("detect_styles: corpus is empty after sampling")
        return []

    prompt_path = Path(__file__).parent / "prompts" / "detect_styles.txt"
    try:
        system_prompt = prompt_path.read_text(encoding="utf-8")
    except FileNotFoundError:
        log.error("detect_styles.txt not found at %s", prompt_path)
        return []

    max_styles_str = str(max_styles) if max_styles > 0 else "no limit"
    user_prompt = (
        f"Maximum styles to identify: {max_styles_str}\n\n"
        f"Writing corpus (stratified sample):\n\n{sample}"
    )

    log.info("Detection pass: calling %s", model_subset or "style-weighted models")
    ...
    if not any_succeeded:
        log.error(
            "detect_styles: all detection models failed; falling back to canonical mode"
        )
        return []
    ...
    if len(successful) == 1:
        ...
        raw_styles = parsed.get("detected_styles", [])
        ...
        if not result_pair:
            return []
        raw_styles, overall_confidence = result_pair
    ...
    # Apply max_styles ceiling
    if max_styles > 0:
        raw_styles = raw_styles[:max_styles]
    ...
    log.info(
        "Detection: found %d style cluster(s): %s",
        len(clusters),
...
def classify_documents(
    ...
    Clusters below per_style_min_words are merged into the nearest cluster.
    """
    if not clusters:
        return {}, list(docs)
    ...
    while changed:
        changed = False
        small = [c for c in clusters if c.word_count < per_style_min_words]
        if not small:
            break
