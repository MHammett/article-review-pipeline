"""Orchestrates multi-model synthesis for all three style modes.

Modes:
  canonical   — M+1 API calls (M synthesis + 1 Claude reconciliation)
  detect      — D+1+(N×M)+1 (detect + consolidate + per-style×models + reconcile)
...
from .style_consolidation import collect_prose, consolidate_lists, DEFAULT_STYLE_WEIGHTS
...
def _build_reconcile_input(
    ...
    for model_name, result in results.items():
        if result.get("failed"):
            continue
        content = result.get("content", "")
        weight = DEFAULT_STYLE_WEIGHTS.get(model_name, 1.0)
    ...
    header = f"Style cluster: {style_label}\n\n" if style_label else ""
    return header + "\n\n".join(sections) + "\n\n" + combined_lists
...
def _detect_mode(
    ...
    """Synthesize per-style profiles and reconcile into canonical + detected_styles."""
    ...
    # Per-style synthesis
    per_style_results: dict[str, dict] = {}
    for cluster in clusters:
        ...
        extra = (
            f"Style cluster: {cluster.label}\n"
            f"Description: {cluster.description}\n"
            f"Source distribution: {cluster.source_distribution}\n"
        )
        style_results = _run_synthesis_pass(
            cluster_docs,
            user_config,
            synthesis_models,
            "synthesize_per_style.txt",
            corpus_budget // max(len(clusters), 1),
            max_parallel,
            f"style_{cluster.label}",
            extra_context=extra,
        )

        any_ok = any(not r.get("failed") for r in style_results.values())
        if not any_ok:
            log.warning("All models failed for style %r; skipping", cluster.label)
            continue

        per_style_results[cluster.label] = {
            "results": style_results,
            "consolidated": {
                "banned_words": consolidate_lists(
                    style_results, "additional_banned_words"
                ),
                "positive_rules": consolidate_lists(
                    style_results, "additional_positive_rules"
                ),
            },
            "cluster": cluster,
        }
    ...
    for style_label, style_data in per_style_results.items():
        for model_name, r in style_data["results"].items():
            reconcile_results[f"{model_name}:{style_label}"] = r
    ...
    # Build detected_styles from per-style data
    detected_styles_out: dict[str, dict] = {}
    recon_detected = reconciled.get("detected_styles") or {}

    for style_label, style_data in per_style_results.items():
        cluster = style_data["cluster"]
        style_results = style_data["results"]
        additional = style_data["consolidated"]

        # Pick best prose from reconciliation or model outputs
        recon_style = recon_detected.get(style_label) or {}
        prose_entries = collect_prose(style_results, "style_profile")
        style_profile_text = recon_style.get("style_profile") or (
            prose_entries[0]["text"] if prose_entries else ""
        )

        detected_styles_out[style_label] = {
            "style_profile": style_profile_text,
            "additional_banned_words": recon_style.get("additional_banned_words")
            or additional["banned_words"],
            "additional_positive_rules": recon_style.get("additional_positive_rules")
            or additional["positive_rules"],
            "style_notes": recon_style.get("style_notes") or "",
    ...
    return {
        ...
        "detected_styles": detected_styles_out,
...
def _per_source_mode(
    ...
    for group_label, group_data in per_source_results.items():
        ...
        recon_style = recon_detected.get(group_label) or {}
        prose_entries = collect_prose(group_results, "style_profile")
        style_profile_text = recon_style.get("style_profile") or (
            prose_entries[0]["text"] if prose_entries else ""
        )

        detected_styles_out[group_label] = {
            "style_profile": style_profile_text,
