"""Tests for detect.py — style detection, classification, cluster validation."""
...
class TestDetectStyles:
    def test_detect_styles_returns_clusters(self):
        """detect_styles: mock detection response → StyleCluster list returned with correct fields."""
        from ci_style_profile.detect import detect_styles, StyleCluster
        ...
        with patch(
            "ci_style_profile.detect.call_all", return_value={"claude": mock_result}
        ):
            clusters = detect_styles(docs, user_config, max_styles=5)

        assert len(clusters) == 2
        labels = [c.label for c in clusters]
        assert "technical analysis" in labels
        assert "direct editorial" in labels
        assert all(isinstance(c, StyleCluster) for c in clusters)

    def test_low_confidence_raises_warning(self):
        """overall_confidence: 'low' → CanonicalFallbackWarning raised."""
        from ci_style_profile.detect import detect_styles, CanonicalFallbackWarning
    ...
    def test_all_models_fail_returns_empty(self):
        ...
        with patch(
            "ci_style_profile.detect.call_all",
            return_value={"claude": {"failed": True, "error": "timeout", "tokens": {}}},
        ):
            result = detect_styles(docs, user_config)
...
class TestClassifyDocuments:
    def test_correct_cluster_assignment(self):
        ...
        from ci_style_profile.detect import StyleCluster, classify_documents
    ...
    def test_undersized_cluster_merged(self):
        """Cluster below per_style_min_words → merged into nearest cluster with warning."""
        from ci_style_profile.detect import StyleCluster, classify_documents
        ...
        assigned, ambiguous = classify_documents(
            all_docs, clusters, per_style_min_words=500
        )

        # small_style had < 500 words → merged into large_style
        assert "small_style" not in {c.label for c in clusters}
