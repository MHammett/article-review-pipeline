"""Merge per-model synthesis outputs into a consolidated voice profile.
...
def _get_weight(model_name: str) -> float:
    return DEFAULT_VOICE_WEIGHTS.get(model_name, 1.0)
...
def collect_prose(
    ...
    Used for voice_profile and audience_* prose passed to reconciliation.
    """
    w = weights or DEFAULT_VOICE_WEIGHTS
    entries = []
    for model_name, result in results.items():
...
def consolidate_detection(
    ...
    return voices, overall_confidence
