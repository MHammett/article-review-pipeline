#!/usr/bin/env python3
"""Voice Profile Bootstrap — CLI entry point.

Usage:
  python voice-profile-bootstrap/bootstrap.py --publication mikehammett --sources wordpress --voice detect
...
def _load_presets() -> dict:
    """Load voice-profile presets from configs/presets.yaml."""
...
def _apply_preset(preset_name: str, presets: dict, sources_cfg: dict, cli_args: argparse.Namespace) -> dict:
    ...
    if cli_args.voice:
        effective["voice_mode"] = cli_args.voice
    if cli_args.max_voices is not None:
        effective["max_voices"] = cli_args.max_voices
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="bootstrap.py",
        description="Synthesize a voice/style profile from your writing corpus.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python voice-profile-bootstrap/bootstrap.py --publication mikehammett --sources wordpress --dry-run
  python voice-profile-bootstrap/bootstrap.py --publication mikehammett --sources wordpress,gmail --voice detect
  python voice-profile-bootstrap/bootstrap.py --output-yaml my_profile.yaml --sources textfiles --voice canonical

Note: --dry-run with --voice detect runs one detection API call (not free).
""",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    voice_mode = effective_cfg.get("voice_mode", "detect")
    ...
    max_voices = int(effective_cfg.get("max_voices", 5))
    per_voice_min_words = int(effective_cfg.get("per_voice_min_words", 2000))
    ...
    if args.dry_run and voice_mode in ("canonical", "per-source"):
    ...
    try:
        ...
        profile = synthesize(
            ...
            per_voice_min_words=per_voice_min_words,
            max_parallel=max_parallel,
    ...
    if args.output_format == "yaml" and isinstance(formatter, PublicationYamlFormatter):
        if output_path and output_path.exists():
            ...
            content = formatter.format(profile, mode=voice_mode)
    else:
        content = formatter.format(profile, mode=voice_mode)
