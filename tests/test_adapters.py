...
def _sse_chat_lines(content, usage=None, finish_reason="stop", extras=None, n=3):
    ...
    for piece in _split(text, n):
        lines.append("data: " + json.dumps({"choices": [{"index": 0, "delta": {"content": piece}}]}))
        lines.append("")
    final = {"choices": [{"index": 0, "delta": {}, "finish_reason": finish_reason}]}
    final["usage"] = usage if usage is not None else {"prompt_tokens": 100, "completion_tokens": 50}
    if extras:
        final.update(extras)
...
def _sse_gemini_lines(content, prompt_tokens=120, completion_tokens=60, n=3):
    ...
    lines = []
    for piece in _split(text, n):
        lines.append("data: " + json.dumps({"candidates": [{"content": {"parts": [{"text": piece}]}}]}))
        lines.append("")
...
class TestOpenAI:
    ...
    def test_streamed_response_accumulated_and_parsed(self):
        ...
        from adapters.review import openai as oai
        content = {"flags": [{"passage": "p", "problem": "x", "suggested_rewrite": "y"}],
                   "low_confidence": ["a", "b"]}
    ...
    def test_usage_tokens_captured_from_final_chunk(self):
        ...
        assert mock_session.post.call_args.kwargs["json"]["stream_options"] == {"include_usage": True}
    ...
    def test_malformed_json(self):
        ...
        with patch("adapters.review.openai.requests.Session") as mock_session_cls:
            mock_session = MagicMock()
            mock_session_cls.return_value = mock_session
            mock_session.post.return_value = _sse_mock(_sse_chat_lines("not json at all"))
            result = oai.call("system", "user", "key")
        assert result["failed"] is True
    ...
    def test_web_search_responses_api_streamed(self):
        ...
        for piece in _split(text, 4):
            lines.append("data: " + json.dumps({"type": "response.output_text.delta", "delta": piece}))
            lines.append("")
        lines.append("data: " + json.dumps({
        ...
        assert "responses" in mock_session.post.call_args.args[0] or \
               "responses" in mock_session.post.call_args.kwargs.get("url", "")
        assert mock_session.post.call_args.kwargs["json"]["stream"] is True
...
class TestMistral:
    def _mock_response(self, content_dict):
        return _sse_mock(_sse_chat_lines(content_dict, usage={"prompt_tokens": 80, "completion_tokens": 40}))
...
class TestGemini:
    ...
    def test_thought_parts_excluded_from_stream(self):
        ...
        lines = [
            ...
            "",
            "data: " + json.dumps({"candidates": [{"content": {"parts": [{"text": text}]}}],
    ...
    def test_no_candidates(self):
        ...
        empty = _sse_mock(["data: " + json.dumps({"candidates": [{"content": {"parts": []}}],
                                                   "usageMetadata": {}})])
        with patch("adapters.review.gemini.requests.Session") as mock_session_cls:
            mock_session = MagicMock()
            mock_session_cls.return_value = mock_session
...
class TestGrok:
    def _mock_response(self, content_dict):
        return _sse_mock(_sse_chat_lines(content_dict, usage={"prompt_tokens": 90, "completion_tokens": 45}))
...
class TestClaude:
    def _mock_response(self, content_dict):
        return _sse_mock(_sse_anthropic_lines(content_dict, input_tokens=100, output_tokens=50))

    def test_successful_call(self):
        from adapters.review import claude
        content = {"flags": [{"passage": "test", "problem": "weak logic", "suggested_rewrite": "stronger"}], "low_confidence": []}
        with patch("adapters.review.claude.requests.Session") as mock_session_cls:
            mock_session = MagicMock()
            mock_session_cls.return_value = mock_session
            mock_session.post.return_value = self._mock_response(content)
    ...
    def test_malformed_json(self):
        ...
        with patch("adapters.review.claude.requests.Session") as mock_session_cls:
            mock_session = MagicMock()
            mock_session_cls.return_value = mock_session
            mock_session.post.return_value = _sse_mock(_sse_anthropic_lines("not json at all"))
            result = claude.call("system", "user", "key")
        assert result["failed"] is True
        assert result["raw"] == "not json at all"
...
class TestPerplexityExtractJson:
    def test_plain_json(self):
        from adapters.review.perplexity import _extract_json
...
class TestConfigLoader:
    ...
    def test_normalize_extended_dict_passthrough(self):
        from config_loader import _normalize_model_configs
        cfg = {"gemini": {"provider": "vertex_ai", "model": "gemini-2.5-flash", "project": "my-proj"}}
    ...
    def test_normalize_dict_without_provider_gets_default(self):
        from config_loader import _normalize_model_configs
        result = _normalize_model_configs({"mistral": {"model": "mistral-large-latest"}})
    ...
    def test_normalize_mixed_forms(self):
        ...
        raw = {
            "openai": "gpt-4o",
            "gemini": {"provider": "vertex_ai", "model": "gemini-2.5-flash", "project": "p"},
        }
    ...
    def test_normalize_preserves_enabled_flag(self):
        from config_loader import _normalize_model_configs
        result = _normalize_model_configs({"grok": {"model": "grok-3-latest", "enabled": False}})
...
class TestProviderDispatch:
    ...
    def test_gemini_default_uses_aistudio(self):
        from adapters.review import gemini
        with patch("adapters.review.gemini._call_aistudio") as mock_ai, \
             patch("adapters.review.gemini._call_vertex") as mock_vx:
            mock_ai.return_value = {"failed": False, "data": {}, "model": "x", "tokens": {}, "elapsed_seconds": 0}
    ...
    def test_gemini_vertex_ai_dispatches_to_vertex(self):
        from adapters.review import gemini
        cfg = {"provider": "vertex_ai", "model": "gemini-2.5-flash", "project": "p", "location": "us-central1"}
        with patch("adapters.review.gemini._call_vertex") as mock_vx, \
             patch("adapters.review.gemini._call_aistudio") as mock_ai:
            mock_vx.return_value = {"failed": False, "data": {}, "model": "x", "tokens": {}, "elapsed_seconds": 0}
    ...
    def test_gemini_model_from_provider_config(self):
        ...
        def fake_aistudio(sys, usr, key, model, **kw):
            ...
            return {"failed": False, "data": {}, "model": model, "tokens": {}, "elapsed_seconds": 0}
        with patch("adapters.review.gemini._call_aistudio", side_effect=fake_aistudio):
            gemini.call("sys", "usr", "key", retry=False, provider_config=cfg)
    ...
    def test_openai_default_uses_openai_backend(self):
        from adapters.review import openai as oai
        with patch("adapters.review.openai._call_openai") as mock_oa, \
             patch("adapters.review.openai._call_azure") as mock_az:
            mock_oa.return_value = {"failed": False, "data": {}, "model": "x", "tokens": {}, "elapsed_seconds": 0}
    ...
    def test_openai_azure_dispatches_to_azure(self):
        ...
        with patch("adapters.review.openai._call_azure") as mock_az, \
             patch("adapters.review.openai._call_openai") as mock_oa:
            mock_az.return_value = {"failed": False, "data": {}, "model": "x", "tokens": {}, "elapsed_seconds": 0}
    ...
    def test_openai_azure_missing_endpoint_returns_failure(self):
        ...
        assert result["failed"] is True
        assert "endpoint" in result["error"].lower() or "deployment" in result["error"].lower()

    # --- Mistral ---

    def test_mistral_default_uses_laplateforme(self):
        from adapters.review import mistral
        with patch("adapters.review.mistral._call_laplateforme") as mock_lp, \
             patch("adapters.review.mistral._call_azure") as mock_az:
            mock_lp.return_value = {"failed": False, "data": {}, "model": "x", "tokens": {}, "elapsed_seconds": 0}
    ...
    def test_mistral_azure_dispatches_to_azure(self):
        ...
        with patch("adapters.review.mistral._call_azure") as mock_az, \
             patch("adapters.review.mistral._call_laplateforme") as mock_lp:
            mock_az.return_value = {"failed": False, "data": {}, "model": "x", "tokens": {}, "elapsed_seconds": 0}
    ...
    def test_grok_accepts_provider_config_without_error(self):
        ...
        with patch("adapters.review.grok.requests.Session") as mock_session_cls:
            ...
            mock_session.post.return_value = _sse_mock(
                _sse_chat_lines(content, usage={"prompt_tokens": 10, "completion_tokens": 5}))
...
class TestHistoryWriteErrors:
    ...
    def test_corrections_write_failure_returns_report_path(self, tmp_path):
        ...
        def selective_fail(path, *args, **kwargs):
            call_count["n"] += 1
            if call_count["n"] == 2:  # second open is the corrections log
...
class TestWordPressTermResolution:
    ...
    def test_integer_ids_pass_through(self):
        from adapters.cms.wordpress import _lookup_term_ids
        result = _lookup_term_ids("http://site.com/wp-json/wp/v2", {}, "categories", [5, 12])
        assert result == [5, 12]

    def test_slug_resolved_to_id(self):
        from adapters.cms.wordpress import _lookup_term_ids
        with patch("adapters.cms.wordpress.requests.get") as mock_get:
            mock_get.return_value = self._mock_term_response(42, "data-centers")
            result = _lookup_term_ids("http://site.com/wp-json/wp/v2", {}, "categories", ["data-centers"])
        assert result == [42]
    ...
    def test_unknown_slug_omitted_with_warning(self):
        ...
        with patch("adapters.cms.wordpress.requests.get") as mock_get:
            ...
            result = _lookup_term_ids("http://site.com/wp-json/wp/v2", {}, "tags", ["nonexistent-tag"])
        assert result == []
    ...
    def test_mixed_ids_and_slugs(self):
        ...
        with patch("adapters.cms.wordpress.requests.get") as mock_get:
            ...
            result = _lookup_term_ids("http://site.com/wp-json/wp/v2", {}, "tags", [7, "energy"])
        assert 7 in result
        assert 99 in result
    ...
    def test_single_string_accepted(self):
        ...
        with patch("adapters.cms.wordpress.requests.get") as mock_get:
            ...
            result = _lookup_term_ids("http://site.com/wp-json/wp/v2", {}, "categories", "tech")
        assert result == [3]

    def test_empty_input_returns_empty(self):
        from adapters.cms.wordpress import _lookup_term_ids
        assert _lookup_term_ids("http://site.com/wp-json/wp/v2", {}, "categories", []) == []
        assert _lookup_term_ids("http://site.com/wp-json/wp/v2", {}, "categories", None) == []
...
class TestHandoffParser:
    ...
    def test_missing_required_field_logs_warning(self):
        ...
        import logging
        from handoff_parser import parse_draft_submission
        # Minimal doc with no Article: field and no DRAFT section
        doc = "DRAFT SUBMISSION HANDOFF\n\nPRIMARY CLAIM\nSome claim\n\nDRAFT\nArticle text here."
        with patch("handoff_parser.log") as mock_log:
...
class TestCheckProviderDispatch:
    ...
    def test_check_openai_calls_correct_url(self):
        ...
        with patch("check.requests.post", return_value=resp) as mock_post:
            result = check.check_openai("key", "gpt-4o")
    ...
    def test_check_openai_azure_uses_api_key_header(self):
        ...
        cfg = {"endpoint": "https://res.openai.azure.com", "deployment": "my-dep", "model": "gpt-4o"}
        with patch("check.requests.post", return_value=resp) as mock_post:
    ...
    def test_check_mistral_azure_uses_bearer_auth(self):
        ...
        cfg = {"endpoint": "https://Mistral-abc.eastus2.inference.ai.azure.com", "model": "mistral-large"}
        with patch("check.requests.post", return_value=resp) as mock_post:
...
_REVIEW_ADAPTERS = [
    ("adapters.review.openai",     "gpt-5.5"),
    ("adapters.review.grok",       "grok-4.20-0309-reasoning"),
    ("adapters.review.mistral",    "mistral-medium-3-5"),
    ("adapters.review.perplexity", "sonar-reasoning-pro"),
    ("adapters.review.claude",     "claude-opus-4-8"),
    ("adapters.review.gemini",     "gemini-2.5-flash"),
]
...
_DEFAULT_READ_GAPS = {
    "adapters.review.openai":     120,
    "adapters.review.grok":       120,
    "adapters.review.mistral":    120,
    "adapters.review.perplexity": 160,
    "adapters.review.claude":     120,
    "adapters.review.gemini":     160,
}
...
class TestStreamingTimeoutAcrossAdapters:
    ...
    def test_streams_with_read_gap_not_inflated_by_timeout_seconds(self, module_name, model):
        import importlib
