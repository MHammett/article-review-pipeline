...
class TestFetchSsrfGuard:
    def test_rejects_private_host_without_fetching(self):
        with patch("analysis.webpage.requests.get") as mock_get:
    ...
    def test_fetches_public_host(self):
        ...
        with patch("analysis.webpage._is_public_host", return_value=True), \
             patch("analysis.webpage.requests.get", return_value=resp) as mock_get:
...
class TestBuildHandoffFromUrl:
    def _patch_fetch(self, html):
        return patch("analysis.webpage.fetch_url", return_value=html)
...
class TestUrlModeFlowsIntoReview:
    ...
    def test_main_url_mode_passes_handoff_to_pipeline(self):
        ...
        with patch.object(sys, "argv", argv), \
             patch("analysis.webpage._is_public_host", return_value=True), \
             patch("analysis.webpage.requests.get", return_value=resp), \
             patch("pipeline.logging.FileHandler"), \
             patch("logging.Logger.addHandler"), \
             patch("pipeline.run_draft_pipeline") as mock_run:
            pipeline.main()
...
class TestRunDraftPipelineAcceptsHandoff:
    ...
    def _patches(self):
        return [
            patch("pipeline._read_handoff_file", side_effect=AssertionError("read a file")),
            patch("pipeline.parse_draft_submission", side_effect=AssertionError("parsed a file")),
            patch("pipeline.load_user_config", return_value={"pipeline": {}}),
            patch("pipeline.load_publication_config", return_value={}),
            patch("pipeline.merge_configs", return_value=self._MIN_CONFIG),
            patch("pipeline.check_model_currency", return_value=self._CURRENCY),
        ]
