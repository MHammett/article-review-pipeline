...
class TestValidateLinks:
    ...
    def test_preserves_url_order(self):
        ...
        with patch("analysis.links._check_http", side_effect=fake_check):
    ...
    def test_internal_url_not_sent_to_wayback(self):
        ...
        with patch("analysis.links.wayback_check") as mock_wb:
    ...
    def test_wayback_stale_days_threaded_through(self):
        ...
        def fake_http(url, timeout):
            return {"status_code": 200, "ok": True, "redirected_to": None}
        with patch("analysis.links._check_http", side_effect=fake_http), \
             patch("analysis.links.wayback_check", return_value={"archived": True}) as mock_wb:
