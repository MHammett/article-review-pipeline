...
class TestWaybackCheck:
    def test_archived_url(self):
        ...
        with patch("adapters.citation.wayback.requests.get", return_value=_mock_response(payload)):
    ...
    def test_network_error_returns_none_archived(self):
        with patch("adapters.citation.wayback.requests.get", side_effect=Exception("timeout")):
            result = wayback.check("https://example.com")
    ...
    def test_archive_url_detected_without_api_call(self):
        ...
        with patch("adapters.citation.wayback.requests.get") as mock_get:
    ...
    def test_non_archive_url_still_queries_api(self):
        ...
        payload = {"archived_snapshots": {}}
        with patch("adapters.citation.wayback.requests.get", return_value=_mock_response(payload)) as mock_get:
