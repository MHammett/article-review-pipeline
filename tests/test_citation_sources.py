...
class TestEiaResolver:
    def test_returns_not_found_when_no_api_key(self):
        from adapters.citation.sources import eia
    ...
    def test_includes_api_key_in_request(self):
        ...
        with patch.dict(os.environ, {"EIA_API_KEY": "testkey123"}), \
             patch("adapters.citation.sources.eia.requests.get", return_value=mock_resp) as mock_get:
...
class TestFredResolver:
    def test_returns_not_found_when_no_api_key(self):
        from adapters.citation.sources import fred
        with patch.dict(os.environ, {}, clear=True):
...
class TestCensusResolver:
    def test_returns_not_found_when_no_api_key(self):
        from adapters.citation.sources import census
        with patch.dict(os.environ, {}, clear=True):
